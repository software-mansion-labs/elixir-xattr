defmodule Xattr.Windows do
	...
	def parse(<<
		namelen :: size(32),
		namez :: binary - size(namelen),
		vallen :: size(32),
		value :: binary - size(vallen),
		rest :: binary>>, parsed) do

		name = binary_part(namez, 0, (byte_size(namez) - 1)) # remove cstr null
	...
	def unparse([{name, value} | rest], binary) do
		if(name =~ "\0", do: throw("name must not contain null bytes")) # name can't contain a null
		namez = name <> <<0>> # Add trailing 0 to make it a c string
		...
		unparse(rest, binary <>
			<<namelen::size(32)>> <>
			binary_part(namez, 0, (namelen-1)) <> <<0>> <>
			<<vallen::size(32)>> <>
			binary_part(value, 0, (vallen)))
	end
