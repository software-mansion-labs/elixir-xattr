use rustler::{Error, NifResult, Atom, OwnedBinary, Env, Binary};
...
#[rustler::nif]
fn xattr_native_nif() -> bool { return true }
...
fn getxattr_nif<'a>(env: Env<'a>, path: &str, name: &str) -> NifResult<(Atom, Binary<'a>)> {
	match xattr::get(path, name) {
		Ok(None) =>
			Err(Error::Term(Box::new(atoms::enoattr()))),
		Ok(Some(val)) =>
			Ok((atoms::ok(), str2bin(env, val.as_slice()))),
		Err(error) =>
			build_error(error)
	}
}
